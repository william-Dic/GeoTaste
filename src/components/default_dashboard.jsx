...
import BrandCategoriesPieChart from './BrandCategoriesPieChart';

const NUM_ROWS = 3;
const NUM_COLS = 6;
const cards = Array.from({ length: NUM_ROWS * NUM_COLS }, (_, i) => i + 1);

// Debug: Log the total number of cards
console.log('Total cards:', cards.length);
console.log('Card numbers:', cards);
      ...
      >
        {cards.map((num) => (
          <div
            ...
            style={{
              background: num === 9 
                ? 'linear-gradient(120deg, #00eaff 20%, #23272f 100%)' 
                : 'linear-gradient(120deg, #23272f 60%, #23272f 100%)',
              borderRadius: '1.2rem',
          ...
          >
            {num === 9 ? (
              <div style={{ color: '#00eaff', fontSize: '1rem', fontWeight: 'bold' }}>
                BRAND CATEGORIES CHART - CARD {num}
              </div>
            ) : (
              `Card ${num}`
            )}
